    ...
    <div class="carousel">
      ...
      >
        <img 
          v-for="(p,i) of pics" 
          :key="i" :src="baseUrl+p" 
          :style="{width:innerWidth+'px',height:0.5*innerWidth+'px'}"
        >
      ...
      <ul class="carouselIndex">
        <!-- 轮播小圆点样式，i==pics.length&&idx==0解决轮播到最后一张图，
        瞬移到第一张图时，i值不匹配任何一个idx值，i值为图片数组长度，且此时为第一张图片，
        定时器让小圆点样式有400ms延迟的问题。i==pics.length&&idx==0可以锁定到这个小圆点 -->
        <li 
...
export default {
  ...
  computed:{
    ...
    loca(){
      console.log(JSON.parse(this.cinema.map))
